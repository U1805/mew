    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm animate-fade-in">
      <div className="bg-[#313338] w-full max-w-md rounded-[4px] shadow-lg flex flex-col overflow-hidden animate-scale-in">
        <div className="p-4 pt-5 pb-3">
          ...
          {categoryName && <p className="text-mew-textMuted text-sm leading-5">{t('channel.create.inCategory', { name: categoryName })}</p>}
        </div>
        <form onSubmit={handleSubmit} className="px-4">
          <div className="mb-4">
            <label className="block text-xs font-bold text-mew-textMuted uppercase mb-2">{t('channel.create.type')}</label>
            <select
              value={type}
            ...
            >
              <option value={ChannelType.GUILD_TEXT}>{t('channel.create.typeText')}</option>
              <option value={ChannelType.GUILD_WEB}>{t('channel.create.typeWeb')}</option>
            </select>
          </div>
          <div className="mb-4">
