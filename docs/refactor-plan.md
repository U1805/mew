...
**结果**：完成了最后的清理工作。迁移了所有剩余组件并删除了旧的 `src/components` 目录。对全局类型进行了最终审查，并将非全局类型移至其对应模块。通过修正 ESLint 配置和修复所有 lint 及 build 错误，确保了代码库的整洁与健康。项目现在已完全符合新的功能驱动架构，并通过了所有最终检验。
...
**最终代码检验**：
*   运行 `pnpm -F frontend lint`：修复所有路径和未使用的导入错误。
*   运行 `pnpm -F frontend build`：确保最终的 TypeScript 编译能够通过。
