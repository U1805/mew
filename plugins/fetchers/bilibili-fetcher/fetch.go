...
// Structs for parsing Bilibili API response
...
type APIItem struct {
	...
	Orig *APIItem `json:"orig,omitempty"` // For forwarded dynamics
}
...
type Opus struct { // Article or Draw
	Title   FlexString `json:"title"`
...
// Draw is the legacy structure for image/text posts when major.type == MAJOR_TYPE_DRAW.
// Some bilibili clients still receive this format instead of MAJOR_TYPE_OPUS.
type Draw struct {
	...
	Pictures    []DrawItem `json:"pictures"` // defensive: some endpoints use `pictures`
}
...
type LiveRcmd struct {
	Content string `json:"content"` // This is a JSON string
}
...
func fetchBiliDynamics(ctx context.Context, client *http.Client, uid string) ([]APIItem, error) {
	...
	cookieHeader, err := requestBiliCookieHeader(ctx, client)
	if err != nil {
		return nil, err
	}

	var lastErr error
	for i := 0; i < 30; i++ { // Retry up to 10 times for -352 error
		...
		if apiResp.Code == -352 {
			...
			if refreshed, err := requestBiliCookieHeader(ctx, client); err == nil && refreshed != "" {
				cookieHeader = refreshed
			}
			select {
			case <-ctx.Done():
				return nil, ctx.Err()
			case <-time.After(time.Second): // Wait a bit before retrying
				continue
			}
		}
