...
func NewClient(useProxy bool) (*Client, error) {
	httpClient, err := sdk.NewHTTPClient(sdk.HTTPClientOptions{
		Timeout:     35 * time.Second,
		CookieJar:   true,
		UseMEWProxy: useProxy,
	})
	if err != nil {
		return nil, err
	}
...
func (c *Client) FetchStories(ctx context.Context, username string) ([]StoryItem, *UserProfile, error) {
	...
	requestHeaders(connectReq, target, ua)
	...
	requestHeaders(handshakeReq, target, ua)
	...
	requestHeaders(confirmReq, target, ua)
	...
	requestHeaders(searchReq, target, ua)
	...
	for attempt := 0; attempt < 12; attempt++ {
		...
		requestHeaders(pollReq, target, ua)
	...
	return nil, nil, fmt.Errorf("searchResult not found (username=%s)", target)
}
