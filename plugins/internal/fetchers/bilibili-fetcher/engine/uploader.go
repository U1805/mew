...
func (u *Uploader) BuildWebhook(ctx context.Context, item source.APIItem) (*sdk.WebhookPayload, error) {
	tCtx := transformContext{
		ctx:          ctx,
		phClient:     u.downloadClient,
		uploadClient: u.uploadClient,
		apiBase:      u.apiBase,
		webhookURL:   u.webhookURL,
		logPrefix:    u.logPrefix,
	}
	return transformItemToWebhook(tCtx, item)
}
...
func parseRichTextNodes(nodes []source.RichTextNode) string {
	var parts []string
...
func richTextToString(v *source.RichText) string {
	if v == nil {
		return ""
	}
...
func drawItemsToPictures(draw *source.Draw) []source.Picture {
	if draw == nil {
		return nil
	}
	...
	out := make([]source.Picture, 0, len(items))
	for _, it := range items {
		...
		out = append(out, source.Picture{URL: u})
	}
	if len(out) == 0 {
		return nil
	}
...
func majorDescString(major *source.Major) string {
	if major == nil {
		return ""
	}
	switch major.Type {
...
func pickDynamicText(dyn source.ModuleDynamic) string {
...
func uploadRemoteToWebhook(tCtx transformContext, remoteURL, fallbackFilename string) (sdk.WebhookAttachment, error) {
	return sdk.UploadRemoteToWebhook(
		...
		remoteURL,
		fallbackFilename,
		source.BiliUserAgent, // keep fixed UA for bilibili domains
	)
}
...
func collectEmojiNodes(v *source.RichText) []localizedEmoji {
...
func localizeEmojisToWebhook(tCtx transformContext, richTexts ...*source.RichText) []localizedEmoji {
...
func majorTitleAndURL(major *source.Major) (string, string) {
	if major == nil {
		return "", ""
	}
	switch major.Type {
...
func transformItemToWebhook(tCtx transformContext, item source.APIItem) (*sdk.WebhookPayload, error) {
	...
	case "DYNAMIC_TYPE_LIVE_RCMD": // Live
		...
		var liveData source.LiveContent
...
func uploadPictures(tCtx transformContext, pics []source.Picture) (origURLs []string, s3Keys []string) {
	if len(pics) == 0 {
		return nil, nil
	}
