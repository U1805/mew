...
func (r *Runner) Run(ctx context.Context) error {
	...
	for i, task := range r.tasks {
		...
		g.Go(func(ctx context.Context) {
			logPrefix := fmt.Sprintf("[tt-bot] bot=%s task=%d user=%s", r.botID, taskIdx, taskCopy.Username)
			...
			w := &Worker{
				...
				firstRun:    true,
				freshState:  tr.Fresh(),
				// Source request + webhook/download/upload budgets.
				fetchTimeout: 40*time.Second + 15*time.Second + 45*time.Second + 90*time.Second,
			}
