...
// Fetcher provides high-level helpers for fetching and grouping channel message history.
type Fetcher struct {
...
func (f *Fetcher) FetchSessionMessages(ctx context.Context, channelID string) (sessionMsgs []sdkapi.ChannelMessage, recordID string, startAt time.Time, err error) {
	f = f.withDefaults()
	var desc []sdkapi.ChannelMessage
	before := ""

	for page := 0; page < f.MaxPages; page++ {
		msgs, err := messages.FetchChannelMessages(ctx, f.HTTPClient, f.APIBase, f.UserToken, channelID, f.PageSize, before)
		if err != nil {
		...
		curCount, boundaryFound := runtime.CurrentSessionCountInDesc(desc, f.SessionGap, f.MaxSessionMessages)
	...
	runtime.ReverseMessagesInPlace(desc)
...
func (f *Fetcher) RecordSearch(ctx context.Context, channelID, recordID string) ([]sdkapi.ChannelMessage, error) {
	f = f.withDefaults()
	...
	for page := 0; page < f.MaxPages; page++ {
		...
		chrono := append([]sdkapi.ChannelMessage(nil), desc...)
		runtime.ReverseMessagesInPlace(chrono)
		sessions := runtime.SplitSessionsChronological(chrono, f.SessionGap)
		if s := runtime.FindSessionByRecordID(sessions, recordID); len(s) > 0 {
			return s, nil
		}
	}
...
func (f *Fetcher) SearchHistory(ctx context.Context, channelID, keyword string, limit int) ([]sdkapi.ChannelMessage, error) {
	keyword = strings.TrimSpace(keyword)
	if keyword == "" {
		return []sdkapi.ChannelMessage{}, nil
	}
	if limit <= 0 {
		limit = 10
	}
	msgs, err := messages.SearchChannelMessages(ctx, f.HTTPClient, f.APIBase, f.UserToken, channelID, keyword, limit, 1)
	if err != nil {
		return nil, err
	}
...
func filterRetractedMessages(msgs []sdkapi.ChannelMessage) []sdkapi.ChannelMessage {
	if len(msgs) == 0 {
		return msgs
	}
	out := msgs[:0]
