...
func NewBotSession(apiBase, accessToken string, httpClient *http.Client) *BotSession {
	...
	if httpClient != nil {
		...
		s.authedClient = &http.Client{
			Transport: auth.NewAuthTransport(baseTransport, s),
			Jar:       httpClient.Jar,
...
func (s *BotSession) Me() sdkapi.User {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.me
}

func (s *BotSession) User(ctx context.Context) (sdkapi.User, error) {
...
func (s *BotSession) ensureLoggedIn(ctx context.Context) error {
	...
	me, tok, err := auth.LoginBot(ctx, s.baseClient, s.apiBase, s.accessToken)
	if err != nil {
		return err
	}
...
func (s *BotSession) Refresh(ctx context.Context) error {
	me, tok, err := auth.Refresh(ctx, s.baseClient, s.apiBase)
	if err != nil {
		return err
	}
	s.mu.Lock()
