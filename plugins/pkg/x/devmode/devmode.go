...
func Dir() string {
	...
	return filepath.Join(state.BaseDir(), "dev")
}
...
// first stack frame outside the plugins/pkg tree. If it can't be derived, it
// returns "unknown".
func ServiceTypeFromCaller() string {
	...
	for skip := 2; skip <= maxSkip; skip++ {
		...
		if strings.Contains(normalized, "/plugins/pkg/") {
			continue
		}
		dir := filepath.Dir(file)
		if group := filepath.Base(dir); group == "fetchers" || group == "agents" {
