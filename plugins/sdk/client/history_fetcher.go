...
func (f *Fetcher) FetchSessionMessages(ctx context.Context, channelID string) (sessionMsgs []ChannelMessage, recordID string, startAt time.Time, err error) {
	f = f.withDefaults()
	var desc []ChannelMessage
	before := ""

	for page := 0; page < f.MaxPages; page++ {
		msgs, err := FetchChannelMessages(ctx, f.HTTPClient, f.APIBase, f.UserToken, channelID, f.PageSize, before)
		if err != nil {
		...
		curCount, boundaryFound := CurrentSessionCountInDesc(desc, f.SessionGap, f.MaxSessionMessages)
...
func (f *Fetcher) RecordSearch(ctx context.Context, channelID, recordID string) ([]ChannelMessage, error) {
	f = f.withDefaults()
	...
	for page := 0; page < f.MaxPages; page++ {
		...
		chrono := append([]ChannelMessage(nil), desc...)
		ReverseMessagesInPlace(chrono)
		sessions := SplitSessionsChronological(chrono, f.SessionGap)
		if s := FindSessionByRecordID(sessions, recordID); len(s) > 0 {
			return s, nil
		}
	}
...
func (f *Fetcher) SearchHistory(ctx context.Context, channelID, keyword string, limit int) ([]ChannelMessage, error) {
	...
	msgs, err := SearchChannelMessages(ctx, f.HTTPClient, f.APIBase, f.UserToken, channelID, keyword, limit, 1)
	if err != nil {
		return nil, err
	}
...
func filterRetractedMessages(msgs []ChannelMessage) []ChannelMessage {
	if len(msgs) == 0 {
		return msgs
	}
	out := msgs[:0]
