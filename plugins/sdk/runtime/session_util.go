...
func ReverseMessagesInPlace(msgs []sdkapi.ChannelMessage) {
...
func SplitSessionsChronological(msgs []sdkapi.ChannelMessage, gap time.Duration) [][]sdkapi.ChannelMessage {
	if len(msgs) == 0 {
		return nil
	}
	out := make([][]sdkapi.ChannelMessage, 0, 8)
	start := 0
	for i := 1; i < len(msgs); i++ {
		prev := msgs[i-1].CreatedAt
		cur := msgs[i].CreatedAt
		if !prev.IsZero() && !cur.IsZero() && cur.Sub(prev) > gap {
			out = append(out, append([]sdkapi.ChannelMessage(nil), msgs[start:i]...))
			start = i
		}
	}
	out = append(out, append([]sdkapi.ChannelMessage(nil), msgs[start:]...))
	return out
}

// FindSessionByRecordID returns the first session containing recordID.
func FindSessionByRecordID(sessions [][]sdkapi.ChannelMessage, recordID string) []sdkapi.ChannelMessage {
...
func CurrentSessionCountInDesc(desc []sdkapi.ChannelMessage, gap time.Duration, maxMessages int) (count int, boundaryFound bool) {
	if len(desc) == 0 {
		return 0, false
	}
	if maxMessages <= 0 {
