...
:::info 参考实现
本文以官方示例 `plugins/internal/fetchers/test-fetcher` 为最小可用范例。
:::
...
1. **拉取实例配置**：服务启动后，按 `serviceType` 通过 `POST /api/bots/bootstrap` 批量拉取 Bot 实例配置（SDK 已封装）。
...
| `content` | 可选，投递的消息内容（会做 `TrimSpace`）。 |
