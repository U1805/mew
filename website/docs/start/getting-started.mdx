...
```ini title="frontend/.env.development"
...
*   🟢 **Backend**: 监听 `http://localhost:3000`
*   🔵 **Frontend**: 监听 `http://localhost:5173` (Vite 默认端口)
...
### 步骤 4: 启动测试 Bot 服务
为了验证机器人生态，我们提供了一个简单的 Go 语言 Bot 示例。

首先编辑相关配置项：
```ini title="plugins/.env.local"
# 请使用与后端配置文件中相同的 MEW_ADMIN_SECRET
MEW_ADMIN_SECRET=replace-this-with-value-in-backend
# BOT 同步配置的间隔时间 (秒)
...
1.  **创建 Webhook**:
    ...
    *   进入频道设置，创建一个 Webhook，并复制其 URL（例如 `http://localhost:3000/api/webhooks/xxx`）。
    *   *注：Bot 将通过此 Webhook 获得向该频道发送消息的权限。*

2.  **注册 Bot**:
    *   进入“用户设置” -> “Bot” 标签页。
    *   点击注册新 Bot，`Service Type` 选择 **`test`**。
...
3.  **配置参数**:
    *   在 `Config` 区域填入以下 JSON 配置：
    ...
    [
      {
        "interval": 60,
        "webhook": "http://localhost:3000/api/webhooks/xxx",
...
### 1. 基础设施配置
编辑 `docker/garage/garage.toml` 文件，填入必要的安全凭证。你可以使用 `openssl` 生成随机密钥：
