...
:::caution S3 路径风格
默认情况下，Mew 对外文件 URL 采用 **Virtual-Hosted** 风格，形如 `http(s)://<bucket>.<S3_WEB_ENDPOINT>/<key>`。  
如果设置了 `MEW_STATIC_URL`，服务端会返回路径式地址（`${MEW_STATIC_URL}/static/<key>`）。
:::
...
# 可选：对外文件访问基址。设置后会返回 ${MEW_STATIC_URL}/static/<key> 形式的 URL
# 例如：http://localhost:151
MEW_STATIC_URL=
...
此方式适合一键部署包含所有依赖（`server`、`client`、`plugins`、`mongo`、`garage`）的完整环境。
...
### 步骤 3: 访问服务
服务启动后，你可以通过以下地址访问：
*   **前端页面**: `http://localhost:151/`
*   **后端 API**: `http://localhost:151/api`（由 client 容器内 Nginx 反向代理）
